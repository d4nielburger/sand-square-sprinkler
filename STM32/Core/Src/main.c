...
int main(void) {
	...
	uint8_t cnt = 0;
	uint8_t oData = 0b01010101;
	while (1) {
		static uint8_t *clData = NULL;

//		cnt ++;
//		if (cnt > 50) {
//			uint8_t shift = oData << 1;
//			oData = shift | (oData & 0b1 << 7) >> 7;
//
//			cnt = 0;
//		}

		BSP_CURRENT_LIMITER_Read();
		//oData = BSP_Signal_Mirror(*clData);
